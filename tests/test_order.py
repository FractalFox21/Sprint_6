...
class TestOrder:
    @allure.title('Оформление заказа и отображение всплывающего окна с сообщением об успешном создании заказа. ')
    @allure.description('Переходим к созданию заказа, вводим данные тестовых пользователей, \
    Ищем на странице уведомление об успешном оформлении заказа.')
    ...
    def test_placing_order_by_using_top_order_button_positive_result(self, driver, user_set):
        ...
        order_page.fill_form_whom(SetUser.set_user[user_set])
        order_page.tap_next()
        order_page.fill_form_about_rent(SetUser.set_user[user_set])
